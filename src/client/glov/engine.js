...
let app_tick_functions_late = [];
export function addTickFuncLate(cb) {
  app_tick_functions_late.push(cb);
}
...
function tick() {
  ...
  for (let ii = 0; ii < app_tick_functions_late.length; ++ii) {
    app_tick_functions_late[ii](dt);
  }
  if (show_fps) {
    camera2d.setAspectFixed(game_width, game_height);
